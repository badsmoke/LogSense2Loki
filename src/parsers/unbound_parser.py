...
UNBOUND_DEBUG_PATTERN = re.compile(
    r'<\d+>1 (?P<timestamp>[\d\-T:+\.]+) (?P<hostname>\S+) unbound \d+ - \[.*?\] '
    r'\[\d+:[^\]]+\] debug: (?P<message>.+)$'
)
...
def parse_unbound_debug_log(log):
    match = UNBOUND_DEBUG_PATTERN.match(log)
    if not match:
        return None
    message = match.group('message')
    if message.startswith('worker request: max UDP reply size modified'):
        return {'_drop': True}
    return {
        'timestamp': match.group('timestamp'),
        'hostname': match.group('hostname'),
